...
use App\Nova\Filters\MatriculaDeletedFilter;
use App\Nova\Actions\RestaurarMatricula;
...
class Matricula extends Resource
{
    ...
    public function filters(NovaRequest $request)
    {
        ...
        if ($user && method_exists($user, 'isSuperAdmin') && $user->isSuperAdmin()) {
            $filters[] = new MatriculaDeletedFilter;
        }
...
{
    $user = $request->user();
    $deletedFilter = $request->filters ? json_decode($request->filters, true) : [];
    
    if (!is_array($deletedFilter)) {
        $deletedFilter = [];
    }

    foreach ($deletedFilter as $filter) {
        if (isset($filter['class']) && strpos($filter['class'], 'MatriculaDeletedFilter') !== false) {
            if ($filter['value'] === 'only_deleted') {
                $query = $query->onlyTrashed();
            } elseif ($filter['value'] === 'with_deleted') {
                $query = $query->withTrashed();
            }
        }
    }

    if ($user && $user->hasRole('admin_edu')) {
        return $query->where('situacao_matricula', 1);
    }
   ...
   public function actions(NovaRequest $request)
    ...
    new AlterarStatusMatricula(),
        ...
        $user = $request->user();
        if ($user && method_exists($user, 'isSuperAdmin') && $user->isSuperAdmin()) {
            $actions[] = new RestaurarMatricula();
        }
